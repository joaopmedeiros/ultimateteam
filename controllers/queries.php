...
function createConfronto($user1,$user2) {
    ...
    SELECT u.idusuario as usuario_idusuario,
        ...
        gp.gols_pro,
            ...
            ((v.vitorias*3) + case when e.empates is null then 0 else e.empates end) as pontos_partidas,
            par.partidasjogas*3 as pontos_possiveis,
            ((v.vitorias*3) + case when e.empates is null then 0 else e.empates end)/(par.partidasjogas*3) as aproveitamento
    FROM usuario as u
    LEFT JOIN 
    (
      ...
      d.derrotas,
       case when e.empates is null then 0 else e.empates end as empates,
        gp.gols_pro,
         gs.gols_sofr as gols_contr,
          par.partidasjogas as partidas,
          ((v.vitorias*3) + case when empates is null then 0 else empates end) as pontos_partidas,
          par.partidasjogas*3 as pontos_possiveis,
          ((v.vitorias*3) + case when empates is null then 0 else empates end)/(par.partidasjogas*3) as aproveitamento
    FROM usuario as u
    LEFT JOIN 
    (
    -- vitorias por usuario
...
function updateRankingOrdenado(){
    ...
    FROM ranking as r 
    ORDER BY aproveitamento desc, partidas desc, (gols_pro-gols_contr) desc ;";
    try {
        $cConexao = new Conexao();
        $pdo = $cConexao->Conectar();
        $q1 = $pdo->prepare($sqlAtualizaRankingOrdenado);
        $q1->execute();
    } catch(PDOException $e){
        die($e->getMessage());
    }
...
//     }
// Testar
function getRankingOrdenado() {
    try {
        $cConexao = new Conexao();
        $pdo = $cConexao->Conectar();
        $q1 = $pdo->prepare("SELECT  ro.pos, u.nome, ro.vitorias, ro.derrotas, CASE WHEN  ro.empates is null then 0 else ro.empates END AS empates, ro.gols_pro, ro.gols_contr,
		 ro.partidas, CASE WHEN  ro.pontos_partidas is null then 0 else ro.pontos_partidas END AS pontos_partidas, 	convert(ro.aproveitamento*100,double) as aproveitamento
		  FROM ranking_ordenado ro INNER JOIN usuario as u on ro.usuario_idusuario = u.idusuario order by pos asc");
		$q1->execute();
...
function getLider(){
    try {
        $cConexao = new Conexao();
        $pdo = $cConexao->Conectar();
        $q1 = $pdo->prepare("SELECT usuario_idusuario FROM ranking_ordenado ORDER BY pos ASC LIMIT 1");
        $q1->execute();
        $result = $q1->fetchAll();
        return $result;
    } catch(PDOException $e) {
...
// Testar
function getqtdMembrosGrupo($idgrupo){
    $sqlGetqtdMembros = "SELECT COUNT(gp.usuario_idusuario) as qtdmembros FROM usuario_grupo gp WHERE gp.grupo_idgrupo = ?";
    try{
        $q1 = $pdo->prepare($sqlGetqtdMembros);
        $q1->bindValue(1, $idgrupo, PDO::PARAM_STR);
        $q1->execute();
        $result = $q1->fetchAll();
        return $result;  
    } catch(PDOException $e) {
...
function getNomeUsuario($idusuario) {
    $sqlGetnomeUsuario = "SELECT u.nome FROM usuario u WHERE u.idusuario = ?";
    try{
...
// Testar
function getPosicaoUsuario($idusuario){
    $sqlGetPosicaoUsuario = "SELECT ro.pos as posicao FROM ranking_ordenado ro WHERE ro.usuario_idusuario =  ?";
    try{
        $q1 = $pdo->prepare($sqlGetPosicaoUsuario);
        $q1->bindValue(1, $idusuario, PDO::PARAM_STR);
        $q1->execute();
        $result = $q1->fetchAll();
        return $result;  
